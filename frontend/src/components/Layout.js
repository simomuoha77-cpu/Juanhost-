...
import { LayoutDashboard, PlusCircle, Database, Globe, Users, Activity, CreditCard, Settings, Shield, LogOut, Bell, Server, Layers } from 'lucide-react';
...
export default function Layout() {
  ...
  useEffect(() => {
    ...
    const i = setInterval(() => notifAPI.list().then(r => setUnreadCount(r.data.unread)).catch(() => {}), 30000);
      ...
      <aside className="sidebar">
        ...
        <div style={{ flex: 1, padding: '8px 0', overflow: 'auto' }}>
          {nav.map((item, i) => item.divider ? (
            <div key={i} className="sidebar-label">{item.label}</div>
          ) : (
        ...
        <div className="sidebar-bottom">
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 10px', marginBottom: 6 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: '0.68rem', color: 'var(--text-muted)', textTransform: 'capitalize' }}>{user?.plan} plan</div>
            </div>
          </div>
          <NavLink to="/dashboard/activity" className="nav-link">
            <Bell size={15} /> Notifications {unreadCount > 0 && <span className="nav-badge">{unreadCount}</span>}
          </NavLink>
          <button onClick={() => { logout(); navigate('/'); }} className="nav-link" style={{ width: '100%', border: 'none', background: 'none', color: 'var(--red)', cursor: 'pointer', textAlign: 'left' }}>
